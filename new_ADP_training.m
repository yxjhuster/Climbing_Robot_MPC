...
u = 20 * (rand(1) - 1/2);
phi_torque = @(X) [1; X(1); X(2); X(3); X(4);
...
FinalW_cost = zeros(length(phi_cost(xk,u)),N); % the parameters for phi, 4 is the size of the state
FinalW_torque = zeros(length(phi_torque(xk)),N);
...
LHS_J = zeros(NoOfEquations,length(phi_cost(xk, u)));

RHS_U = zeros(NoOfEquations,1);
LHS_U = zeros(NoOfEquations,length(phi_torque(xk)));
%% Training
for t = 0 : N-1
    k = N - t;
...
%             LHS_J(i,:) = phi_cost(xk, 0)'; %not working because it is not invertible. one possible solution is to use phi_torque instead of phi_cost.
            ...
            RHS_J(i,:) = J_k_t;
            LHS_J(i,:) = phi_cost(xk, umax)';
            
            RHS_U(i,:) = umax;
            LHS_U(i,:) = phi_torque(xk)';
        end
    end
    ...
    if k == N
        ...
        FinalW_torque(:,k) = zeros(length(phi_torque(xk)),1);
    else
